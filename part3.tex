...
системы LearnSys, разработанной на основе фреймворка Django~\cite{django2026docs}. 
Система реализована в виде монолитного веб-приложения с разделением на 
несколько логических модулей: модуль авторинга курсов, модуль 
психодиагностики, адаптивное тестирование, трекинг активности, 
пользовательские интерфейсы и аналитика. Все модули взаимодействуют 
через единую объектно-реляционную базу данных и общий слой бизнес-логики.

Архитектура системы построена на принципе разделения ответственности: 
модели данных описывают предметную область и хранятся в модуле 
\texttt{learnsys/models.py}, представления обрабатывают HTTP-запросы 
и содержатся в \texttt{learnsys/views.py}, а шаблоны формируют 
пользовательский интерфейс. Вспомогательная логика вынесена в 
утилиты: \texttt{learnsys/utils/}, \texttt{learnsys/psych\_utils.py}, 
\texttt{learnsys/recommender.py} и \texttt{learnsys/mouse\_metrics.py}.
...
\texttt{learnsys/utils/\_\_init\_\_.py}.
...
Автоматическое соединение тем реализовано в функции 
\texttt{auto\_connect\_topics}. При включённом режиме 
...
\texttt{learnsys/utils/content\_processing.py}. Функция 
\texttt{process\_content(content\_instance)} вызывается после сохранения 
...
\texttt{calculate\_factors(user\_answers, test, target\_factors)} 
в модуле \texttt{learnsys/utils/calculate\_factors.py}. Параметр 
\texttt{user\_answers} представляет собой словарь вида 
\texttt{\{question\_id: answer\_id\}}.

Алгоритм выполняется в шесть этапов.

\textbf{Этап 1: Загрузка вопросов.} Загружаются вопросы теста с 
предзагрузкой вариантов ответов через \texttt{prefetch\_related} для 
минимизации числа SQL-запросов.

\textbf{Этап 2: Накопление баллов.} Для каждого вопроса определяется 
...
\textbf{Этап 3: Расчёт темперамента.} Фактор \texttt{A4} не вычисляется 
...
if a3_score < 12 and a2_score < 12:
    factor_scores['A4'] = 1   # Phlegmatic
elif a3_score < 12 and a2_score >= 12:
    factor_scores['A4'] = 2   # Sanguine
elif a3_score >= 12 and a2_score < 12:
    factor_scores['A4'] = 3   # Melancholic
else:
    factor_scores['A4'] = 4   # Choleric
\end{lstlisting}
...
из \texttt{learnsys/utils/psych\_factors.py}. Функция возвращает словарь 
...
в модуле \texttt{learnsys/psych\_utils.py}. Функция принимает объект 
...
\texttt{get\_geo(ip)} из \texttt{learnsys/recommender.py}, 
...
в \texttt{learnsys/signals.py} подключён через декоратор 
...
функциями, вынесенными в \texttt{learnsys/views.py}: 
\texttt{\_get\_a1\_interpretation(value)}, 
\texttt{\_get\_a2\_interpretation(value)}, 
\texttt{\_get\_a3\_interpretation(value)}, 
\texttt{\_get\_a4\_interpretation(value)}.
...
\texttt{learnsys/mouse\_metrics.py}. Функция принимает список 
...
\texttt{learnsys/views\_analytics.py}. Панель агрегирует данные по всем 
...
\textbf{Статистика курсов.} Для каждого курса, доступного студенту 
...
\textbf{Психологический профиль.} Передаётся в контекст \\как 
...
\textbf{Блок рекомендаций.} На дашборде реализована динамическая 
...
API-эндпоинту \texttt{/learnsys/api/recommendations/}.
...
\subsubsection{Рекомендательная подсистема}

Полная цепочка рекомендаций курсов реализована в функции 
\texttt{recommend\_courses(user, ip, limit)} модуля 
\texttt{learnsys/recommender.py}. Функция последовательно выполняет: 
определение геолокации $\to$ получение погоды $\to$ загрузку последних 
значений факторов через \texttt{PsychTestResult} $\to$ применение 
погодных, временных и поведенческих коррекций $\to$ интерпретацию 
факторов в психологические теги через словарь 
\texttt{INTERPRETATIONS} $\to$ маппинг тегов на теги курсов через 
\texttt{FACTOR\_TAGS} $\to$ поиск опубликованных курсов по тегам $\to$ 
ранжирование по релевантности.

Временные коррекции вычисляются функцией \texttt{\_get\_time\_factor}: 
утренние часы (6:00--10:00) повышают \texttt{A1} на 1 и снижают 
\texttt{A3} на 1; послеобеденные часы (14:00--16:00) снижают 
\texttt{A1} на 1; ночные часы (22:00--5:00) снижают \texttt{A1} на 2 
и повышают \texttt{A3} на 2. Дополнительно понедельник повышает 
\texttt{A3} на 1, пятница снижает \texttt{A1} на 1.

Для объяснения рекомендаций пользователю служит функция 
\texttt{explain\_recommendation(course, user\_scores)}, формирующая 
список текстовых объяснений на основе совпадений тегов курса с 
интерпретациями факторов студента.

\subsubsection{Критерии эффективности}

Для оценки результатов эксперимента использовались следующие критерии.

\textbf{Учебные результаты.} Сравнивались средние значения поля 
...
\textbf{Вовлечённость.} Измерялась через поле \texttt{activity\_score} 
...
\textbf{Удовлетворённость.} Оценивалась по опросникам, заполняемым 
...
Анализ данных показал положительную динамику в экспериментальной 
группе по сравнению с контрольной. Среднее значение итогового 
балла (\texttt{percentage}) в экспериментальной группе оказалось 
выше, что свидетельствует о позитивном влиянии адаптивного порядка 
изучения тем на учебные результаты. Данные модели \texttt{TopicProgress} 
показали более высокую долю завершённых тем в экспериментальной 
группе, что указывает на повышение вовлечённости студентов.

Анализ значений \texttt{mouse\_motivation\_score} подтвердил 
корреляцию поведенческих метрик с психометрическими данными: 
у студентов с высоким фактором \texttt{A1} (мотивация на успех, 
значение $\geq 14$) наблюдались более высокие значения 
поведенческой метрики мотивации. Опросы показали, что большинство 
участников экспериментальной группы положительно оценили 
персонализированный порядок тем и воспринимали рекомендации как 
релевантные собственным учебным потребностям.

Полученные результаты согласуются с теоретическими положениями 
о преимуществах адаптивных обучающих систем~\cite{brusilovsky2001adaptive, woolf2009building} 
и подтверждают работоспособность реализованной архитектуры 
психологического профилирования с контекстными коррекциями.

\subsubsection{Ограничения исследования}

В ходе эксперимента были выявлены следующие ограничения. Размер 
выборки ограничен вследствие специфики учебной среды, что снижает 
статистическую мощность выводов. Погодные коррекции факторов 
основаны на усреднённых коэффициентах и не учитывают индивидуальные 
различия в реакции студентов на погодные условия. Трекинг движений 
мыши как метод оценки психологических характеристик является 
косвенным и требует дополнительной валидации. В дальнейших 
исследованиях планируется расширение выборки, введение более 
детализированной модели поведенческих метрик и интеграция 
инструментов глубокого обучения для повышения точности предсказания 
учебных результатов.